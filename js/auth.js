...
export function useAuth(transactions, shopProfile, updateCharts) {
    ...
    const logout = () => {
        ...
        fetch(`${API_BASE_URL}/api/auth/logout`, { method: 'POST', credentials: 'include', headers: csrf ? { 'X-CSRF-Token': csrf } : {} }).catch(() => {});
    ...
    const changeUsername = async (newUsername) => {
        try {
            ...
            const csrf = getCsrfToken();
            const res = await fetch(`${API_BASE_URL}/api/auth/change-username`, {
                method: 'POST',
